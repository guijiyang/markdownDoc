...
[生命周期](https://en.cppreference.com/w/cpp/language/lifetime)
...
此外，在无名命名空间或无名命名空间内的命名空间中声明的所有名称，即使显式声明为extern，也具有内部链接。（自c++11起）。
...
约束是由逻辑操作和操作数组成的序列，用于指定对模板实参的要求。 它们可以出现在需求(requires)表达式中，也可以直接作为概念主体。
约束有三种类型：
...
原子约束由一个表达式 E 和一个映射组成，该映射将出现在 E 中的模板形式参数映射到涉及受限实体模板形式参数的模板实参。
原子约束是在约束规范化过程中形成的。E 永远不会是一个逻辑与（logical AND）或逻辑或（logical OR）表达式（分别形成合取和析取）。
通过将参数映射和模板实参代入表达式 E 来检查原子约束的满足情况。如果代入结果导致无效的类型或表达式，则约束不满足。否则，在任何左值到右值的转换之后，E 必须是一个 bool 类型的prvalue常量表达式，并且只有当它评估为真时，约束才被认为是满足的。
在代入后，E 的类型必须是确切的 bool 类型。不允许进行任何转换：
...
```
如果两个原子约束在源代码级别由相同的表达式形成，并且它们的参数映射是等价的，那么这两个原子约束被认为是相同的。
